...
> ⚠️ **变更提示（2026-08-11）**：本文中"弱触发经验存入 MySQL `experience_temp` 临时表（TTL）"的设计已废弃。
> 弱触发经验现与强经验同路径入 Milvus（tier=weak、置信度打折）参与召回，评分成功后可晋升 strong；
> 召回评分也已引入 recency 时间因子，且高相似经验会合并更新而非新增。
> 详见《更新日志-经验闭环与记忆增强.md》。
